...
Camera camera(winWidth, winHeight, vec3(0, 180, 0), vec3(0, 0, -5), 30);
...
void Display(GLFWwindow *window) {
	...
	VertexAttribPointer(program, "uv", 2, 0, (void *)(2 * sizeof(points)));
	...
	float	dx = .48,
			...
			sy = .41,
			sz = .5;
	mat4 t = Translate(dx, dy, 0) * Scale(sx, sy, sz);
...
void initUvs() {
	for (int i = 0; i < npoints; i++) {
		uvs[i] = vec2(points[i].x, points[i].y);
	}
}
...
int main() {
    ...
    if (!glfwInit())
        return 1;
    GLFWwindow *w = glfwCreateWindow(winWidth, winHeight, "Textured Face", NULL, NULL);
    if (!w) {
        glfwTerminate();
        return 1;
    }
	...
	if (!InitShader())
        return 0;
	textureName = LoadTexture("C:/Users/Toaster 3.0/Desktop/Graphics/gramp.tga", textureUnit);
	InitNormals();
	initUvs();
    InitVertexBuffer();
