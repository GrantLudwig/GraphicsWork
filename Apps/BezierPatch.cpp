...
vec3 BezPoint(float t, vec3 b1, vec3 b2, vec3 b3, vec3 b4) {
	...
	return	(pow(-t, 3.0) + (3 * pow(t, 2.0)) - (3 * t) + 1) * b1 + // b1
		((3 * pow(t, 3.0)) - (6 * pow(t, 2.0)) + (3 * t)) * b2 + // b2
		((-3 * pow(t, 3.0)) + (3 * pow(t, 2.0))) * b3 + // b3
		pow(t, 3.0) * b4; // b4
}
...
void MouseMove(GLFWwindow *w, double x, double y) {
    ...
    if (glfwGetMouseButton(w, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) { // drag
		shift = glfwGetKey(w, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS ||
