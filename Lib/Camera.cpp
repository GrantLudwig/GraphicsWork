...
Camera::Camera(int screenWidth, int screenHeight, vec3 rot, vec3 tran, float fov, float nearDist, float farDist, bool invVrt) :
	aspectRatio((float) screenWidth/screenHeight), rot(rot), tran(tran), fov(fov),
	nearDist(nearDist), farDist(farDist), invertVertical(invVrt) {
		tranSpeed = .01f;
		rotSpeed = .3f;
		persp = Perspective(fov, aspectRatio, nearDist, farDist);
